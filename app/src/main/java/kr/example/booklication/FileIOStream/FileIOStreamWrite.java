package kr.example.booklication.FileIOStream;
...
public class FileIOStreamWrite {
    ...
    public void reportWriteData(String fileName, String writeData) {
        // 기본 경로로 설정함
        String sDataFilePath = aFileIOStreamWrite.getFilesDir().getAbsolutePath()+"/bookreport/";
        File fDataFile = new File(sDataFilePath + fileName);    // 파일 이름으로 생성

        System.out.println("report fileName : " + fileName);
        System.out.println("report path : " + fDataFile.toString());
        System.out.println("report WriteData : " + writeData);
