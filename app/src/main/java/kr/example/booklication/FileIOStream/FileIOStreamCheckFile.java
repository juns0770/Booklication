...
//        fFile.delete();
    ...
    public void reportCheckFile(String fileName) {
        String sFilePath = aFileIOStreamCheckFile.getFilesDir().getAbsolutePath()+"/bookreport/";
        File fFile = new File(sFilePath + fileName);    // 파일 이름으로 생성
        System.out.println("report path : " + fFile.toString());
        ...
        if(!fFile.exists()) {
            try{
                ...
                FileOutputStream fosDataFile = new FileOutputStream(fFile, false);
                String sTempData = "";
                fosDataFile.write(sTempData.getBytes());
                fosDataFile.close();
                System.out.println("텍스트파일 생성완료");
